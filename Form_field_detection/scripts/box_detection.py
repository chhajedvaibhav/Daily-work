...
#considering first image only rest process would be same
for i in range(len(images)):
    print(os.getcwd())
    image_name = images[i]
    images[i] = '../images/' + images[i]
    pages = convert_from_path(images[i], 500)
    ...
    cropped_images = 'cropped_images' + image_name
    create_directory(cropped_images)
        
    
    os.chdir(cropped_images)
    idx = 0
    for c in contours:
